...
1. 乘法，由一个星号（`*`）表示，用于计算两个数值的乘积，都先进行 `Number()` 转换，在计算不可计算返回 `NaN`

```JavaScript
var result = 34 * 56;
```

2. 除法，由一个斜线符号（`/`）表示，用于计算两个数的商，都先进行 `Number()` 转换，在计算不可计算返回 `NaN`

```JavaScript
var result = 66 / 11;
```

3. 求模,由一个百分号（`%`）表示，用于计算两个数的余数，都先进行 `Number()` 转换，在计算不可计算返回 `NaN`
