...
1. 变量赋值语句，创建一个函数并将它赋值给变量 `functionName`
...
2. 当调用 compare()时，会 创建一个包含 arguments、value1 和 value2 的活动对象，活动对象的作用域在该函数内部
...
2. 外部作用域中的 this 对象保存在一个闭包能够访问 到的变量里，就可以让闭包访问该对象
...
> 定义局部变量，函数执行完毕后，没有指向匿名函数的引用。只要函 数执行完毕，就可以立即销毁其作用域链了
...
1. 匿名函数返回对象，限制内部变量操作，通过方法设置访问赋值同一个内部变量
...
var application = (function() {
  ...
  //创建 application 的一个局部副本 var app = new BaseComponent();
  //公共接口
